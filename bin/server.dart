...
import 'package:dart_ipify/dart_ipify.dart' as di;
...
Future<Response> loginHandler(Request req) async {
  ...
  return Response.ok(
      jsonEncode({'result': res.length > 0 ? 'succesful' : 'failed'}));
}
...
Future<Response> signUpHandler(Request req) async {
  ...
  return Response.ok(jsonEncode(
      {'result': res.affectedRows! < 1 ? 'failed' : 'user created'}));
}
...
void main(List<String> args) async {
  ...
  String deviceIp=await di.Ipify.ipv4();
print('ipppp>>>$deviceIp');
